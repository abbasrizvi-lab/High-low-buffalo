...
const ConnectionList = ({ connections, type }: ConnectionListProps) => {
  ...
  return (
    <ul className="space-y-3 mt-4">
      {connections.map((connection) => (
        <li key={connection.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50">
          <div className="flex items-center gap-3">
            <Avatar>
              <AvatarImage src={connection.user.avatarUrl} alt={connection.user.name} />
